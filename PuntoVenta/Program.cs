...
List<Venta> VentasDia = new List<Venta>();
...
do
{
    ...
    if (opcion == 1)
    {
        ...
        do
        {
            ...
            DetalleInventario detalleInv = Inventario.BuscarPorCodigoBarras(codigoBarras);
            if (detalleInv == null)
            {
    ...
    else if (opcion == 2)
    {
        ...
        for (int i = 0; i <Inventario.DetalleInventario.Count(); i++)
        {
            Console.WriteLine(Inventario.DetalleInventario[i]);
        }
    }
    else if (opcion == 3)
    {
        ...
        bool productoAgregado = Inventario.AgregarProducto(codigoBarras, descripcion, precio, cantidad);
        if (productoAgregado)
